...
const createDefaultEnabledState = (): EnabledState => ({
  ...
  priorityRulesEnabled: DEFAULT_ENABLED_STATE.priorityRulesEnabled,
});
...
const hasNewEnabledStateFields = (state: PersistedEnabledState) => {
  return "pluginEnabled" in state || "priorityRulesEnabled" in state;
};
...
const createResultFromParsedState = (raw: PersistedEnabledState): ReadEnabledStateResult => {
  if (hasNewEnabledStateFields(raw)) {
    ...
    const priorityRulesEnabledValid = typeof raw.priorityRulesEnabled === "boolean";
    const state = {
      pluginEnabled: pluginEnabledValid
        ? (raw.pluginEnabled as boolean)
        : DEFAULT_ENABLED_STATE.pluginEnabled,
      priorityRulesEnabled: priorityRulesEnabledValid
        ? (raw.priorityRulesEnabled as boolean)
        : DEFAULT_ENABLED_STATE.priorityRulesEnabled,
    } satisfies EnabledState;

    if (pluginEnabledValid && priorityRulesEnabledValid) {
  ...
  if (typeof raw.enabled === "boolean") {
      return {
        state: {
          ...
          priorityRulesEnabled: raw.enabled as boolean,
        },
        kind: "legacy",
        raw,
      };
...
export const readEffectiveEnabledState = async (context: EnabledStateContext) => {
  ...
  return state.pluginEnabled && state.priorityRulesEnabled;
};
...
const createPersistedState = (
  ...
  return {
    ...
    priorityRulesEnabled: state.priorityRulesEnabled,
  };
};
...
export const writePriorityRulesEnabledState = async (
  context: EnabledStateContext,
  priorityRulesEnabled: boolean,
) => {
  await writeMergedEnabledState(context, { priorityRulesEnabled });
};
