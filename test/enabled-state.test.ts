...
const STATE_FILE_PARTS = ["plugin-data", "opencode-priority-rule-plugin", "state.json"] as const;
...
describe("enabled state helpers", () => {
  ...
  it("returns both flags enabled when the state file is missing", async () => {
    ...
    ).resolves.toEqual({ pluginEnabled: true, priorityRulesEnabled: true });
  });
  ...
  it("reads the new dual-state format", async () => {
    ...
    await writeRawStateFile(
      configDir,
      JSON.stringify({ pluginEnabled: false, priorityRulesEnabled: true }),
    );

    await expect(
      readEnabledState({ directory: configDir, worktree: configDir }),
    ).resolves.toEqual({ pluginEnabled: false, priorityRulesEnabled: true });
  });
  ...
  it("supports the legacy enabled format", async () => {
    ...
    await expect(
      readEnabledState({ directory: configDir, worktree: configDir }),
    ).resolves.toEqual({ pluginEnabled: false, priorityRulesEnabled: false });
  });
  ...
  it("prefers new-format fields over legacy enabled when both formats appear", async () => {
    ...
    await writeRawStateFile(
      configDir,
      JSON.stringify({ enabled: false, priorityRulesEnabled: true }),
    );
  ...
  it("falls back each invalid new-format field independently", async () => {
    ...
    await writeRawStateFile(
      configDir,
      JSON.stringify({ pluginEnabled: false, priorityRulesEnabled: "yeah totally" }),
    );
  ...
  it("computes the effective enabled state from both flags", async () => {
    ...
    await writeRawStateFile(
      configDir,
      JSON.stringify({ pluginEnabled: true, priorityRulesEnabled: false }),
    );
  ...
  it("writes the enabled state JSON to the resolved file", async () => {
    ...
    expect(JSON.parse(contents)).toEqual({ pluginEnabled: false, priorityRulesEnabled: false });
  });
  ...
  it("writePluginEnabledState merges without overwriting priorityRulesEnabled", async () => {
    const configDir = await trackTempDir("enabled-config-");
    const context = { directory: configDir, worktree: configDir };
  ...
  it("writePriorityRulesEnabledState only changes priorityRulesEnabled", async () => {
    const configDir = await trackTempDir("enabled-config-");
    const context = { directory: configDir, worktree: configDir };
    ...
    await writePriorityRulesEnabledState(context, false);
  ...
  it("reports legacy state without warning", async () => {
    ...
    await expect(
      ...
      kind: "legacy",
      state: { pluginEnabled: false, priorityRulesEnabled: false },
      warning: undefined,
      raw: { enabled: false },
    });
  });
  ...
  it("reports malformed JSON as an invalid-file warning", async () => {
    ...
    await expect(
      ...
      state: { pluginEnabled: true, priorityRulesEnabled: true },
      raw: undefined,
    });
  });
  ...
  it("reports partial-invalid-fields when new-format values need fallback", async () => {
    ...
    await writeRawStateFile(
      configDir,
      JSON.stringify({ pluginEnabled: false, priorityRulesEnabled: "broken", futureSetting: true }),
    );
    ...
    await expect(
      ...
      state: { pluginEnabled: false, priorityRulesEnabled: true },
      raw: { pluginEnabled: false, priorityRulesEnabled: "broken", futureSetting: true },
    });
  });
  ...
  it("writePriorityRulesEnabledState preserves unknown top-level fields from parseable JSON", async () => {
    const configDir = await trackTempDir("enabled-config-");
    const context = { directory: configDir, worktree: configDir };
    ...
    await writeRawStateFile(
      configDir,
      JSON.stringify({ pluginEnabled: true, priorityRulesEnabled: true, futureSetting: "keep-me" }),
    );
  ...
  it("writePriorityRulesEnabledState preserves unknown top-level fields when they are the only parseable keys", async () => {
    const configDir = await trackTempDir("enabled-config-");
    const context = { directory: configDir, worktree: configDir };
  ...
  it("writePriorityRulesEnabledState self-heals malformed JSON into the new format", async () => {
    const configDir = await trackTempDir("enabled-config-");
    const context = { directory: configDir, worktree: configDir };
  ...
  it("throws write errors instead of swallowing them", async () => {
    ...
    await expect(
      writePriorityRulesEnabledState({ directory: blockedPath, worktree: blockedPath }, false),
    ).rejects.toBeDefined();
  });
});
