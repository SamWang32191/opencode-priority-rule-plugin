...
import { PRIORITY_RULES } from "../src/priority-rules.js";
...
describe("server plugin", () => {
  ...
  it("injects the priority rules before the first user part", async () => {
    ...
    expect(output.messages[1].parts[0]).toEqual({ type: "text", text: PRIORITY_RULES });
  ...
  it("does not inject the tone twice", async () => {
    ...
    const output = createOutput([
      {
        info: createUserInfo(),
        parts: [createTextPart(PRIORITY_RULES), createTextPart("hello")],
      },
    ]);
    ...
    expect(output.messages[0].parts[0]).toMatchObject({ type: "text", text: PRIORITY_RULES });
    expect(output.messages[0].parts[1]).toMatchObject({ type: "text", text: "hello" });
  });

  it("does not mistake user content containing PRIORITY_RULES for an existing injection", async () => {
    process.env.OPENCODE_CONFIG_DIR = await trackTempDir("server-config-");
    const transform = await createTransform();
    const output = createOutput([
      {
        info: createUserInfo(),
        parts: [createTextPart(`user quoted this: ${PRIORITY_RULES}`)],
      },
    ]);

    await transform(TRANSFORM_INPUT, output);

    expect(output.messages[0].parts[0]).toEqual({ type: "text", text: PRIORITY_RULES });
    expect(output.messages[0].parts[1]).toMatchObject({
      type: "text",
      text: `user quoted this: ${PRIORITY_RULES}`,
    });
  });

  it("inserts a clean priority-rules part without copying extra fields", async () => {
    process.env.OPENCODE_CONFIG_DIR = await trackTempDir("server-config-");
    const transform = await createTransform();
    const output = createOutput([
      {
  ...
  it("does not inject when priorityRulesEnabled is false", async () => {
    const configDir = await trackTempDir("server-config-");
    process.env.OPENCODE_CONFIG_DIR = configDir;
    await writeStateFile(
      configDir,
  ...
  it("re-reads enabled state on every transform call", async () => {
    ...
    expect(enabledOutput.messages[0].parts[0]).toEqual({ type: "text", text: PRIORITY_RULES });
  });
});
