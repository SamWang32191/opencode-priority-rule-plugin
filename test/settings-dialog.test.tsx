...
const mountDialog = (options?: {
  ...
  const [value, setValue] = createSignal<SettingsState>(options?.initialValue ?? { priorityRulesEnabled: true });
...
describe("SettingsDialog", () => {
  it("renders the priority rules row with filter, category, and ON footer", () => {
  ...
  it("routes Enter through DialogSelect onSelect", async () => {
    ...
    expect(dialog.flip).toHaveBeenCalledWith("priorityRulesEnabled");
  ...
  it("routes Space and arrow keys through useKeyboard", () => {
    ...
    expect(dialog.flip).toHaveBeenNthCalledWith(2, "priorityRulesEnabled");
    expect(dialog.flip).toHaveBeenNthCalledWith(3, "priorityRulesEnabled");

    dialog.dispose();
  });

  it("shows Saving... and disables all rows while a save is in flight", () => {
    const dialog = mountDialog({ initialSavingField: "priorityRulesEnabled" });
