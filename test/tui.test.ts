...
vi.mock("../src/settings-dialog.js", () => ({
  SettingsDialog: (props: {
    ...
    value: () => { priorityRulesEnabled: boolean };
    savingField: () => string | undefined;
    flip: (field: "priorityRulesEnabled") => void | Promise<void>;
  }) => {
    return props.api.ui.DialogSelect({
      title: "Priority Rules settings",
      placeholder: "Filter settings",
      current: "priorityRulesEnabled",
      options: [
        {
          title: "Priority rules enabled",
          value: "priorityRulesEnabled",
          description: "Apply priority rules to future messages.",
          category: "Priority Rules",
          footer: props.savingField() === "priorityRulesEnabled" ? "Saving..." : props.value().priorityRulesEnabled ? "ON" : "OFF",
          disabled: props.savingField() !== undefined,
        },
      ],
      onSelect: async (option) => {
        if (option.value !== "priorityRulesEnabled") {
          return;
        }

        await props.flip("priorityRulesEnabled");
      },
    });
  },
}));
...
const TEST_PLUGIN_ID = "opencode-priority-rule-plugin";
...
const readStateFile = async (context: EnabledStateContext) => {
  ...
  return JSON.parse(contents) as { pluginEnabled: boolean; priorityRulesEnabled: boolean };
};

const writeRawStateFile = async (
  context: EnabledStateContext,
  state: { pluginEnabled: boolean; priorityRulesEnabled: boolean },
) => {
...
describe("tui entrypoint", () => {
  it("registers the Priority Rules settings command with a clarifying description", async () => {
    ...
    expect(plugin.commands()).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          title: "Priority Rules settings",
          description: "Enable or disable priority-rules injection without disabling the plugin",
          value: "priority-rules-settings",
          category: "Plugin",
        }),
      ]),
    );
  ...
  it("writes pluginEnabled=true and priorityRulesEnabled=true when the plugin loads with no prior state", async () => {
    const configDir = await trackTempDir("tui-config-");
    const worktree = await trackTempDir("tui-worktree-");
    const context = { directory: worktree, worktree };
  ...
  it("preserves priorityRulesEnabled when the plugin loads", async () => {
    const configDir = await trackTempDir("tui-config-");
    const worktree = await trackTempDir("tui-worktree-");
    const context = { directory: worktree, worktree };

    process.env.OPENCODE_CONFIG_DIR = configDir;
    await writeRawStateFile(context, { pluginEnabled: false, priorityRulesEnabled: false });
  ...
  it("opens the TSX settings dialog at medium size", async () => {
    ...
    const command = plugin.commands().find((entry) => entry.title === "Priority Rules settings");
  ...
  it("rolls back and shows an error toast when saving priorityRulesEnabled fails", async () => {
    const blockedPath = join(await trackTempDir("tui-blocked-"), "blocked-file");
    const context = { directory: blockedPath, worktree: blockedPath };
  ...
  it("preserves the optimistic value and dialog lock when reopened during an in-flight save", async () => {
    ...
    const originalWritePriorityRulesEnabledState = enabledState.writePriorityRulesEnabledState;
    const writePriorityRulesEnabledState = vi
      .spyOn(enabledState, "writePriorityRulesEnabledState")
      .mockImplementation(async (nextContext, priorityRulesEnabled) => {
        expect(nextContext).toEqual(context);
        expect(priorityRulesEnabled).toBe(false);
        await writePending;
        await originalWritePriorityRulesEnabledState(nextContext, priorityRulesEnabled);
      });
    ...
    expect(writePriorityRulesEnabledState).toHaveBeenCalledTimes(1);
    ...
    writePriorityRulesEnabledState.mockRestore();
  });
  ...
  it("only writes pluginEnabled=false when dispose happens after the plugin is disabled", async () => {
    ...
    process.env.OPENCODE_CONFIG_DIR = configDir;
    await writeRawStateFile(context, { pluginEnabled: true, priorityRulesEnabled: true });
  ...
  it("does not corrupt state when dispose happens but the plugin remains enabled", async () => {
    ...
    process.env.OPENCODE_CONFIG_DIR = configDir;
    await writeRawStateFile(context, { pluginEnabled: true, priorityRulesEnabled: false });
